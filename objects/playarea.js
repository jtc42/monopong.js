import { monoDB } from "../db.js";
import { Ball } from "./ball.js";
import { Batton } from "./batton.js";
...
import { refreshScores } from "../logic/scores.js";
import { keyClass } from "../logic/keyboard.js";
...
export class PlayArea {
  constructor(viewWidth, viewHeight) {
    ...
    this.gameActive = false; //Game active
    ...
    this.keysDown = {};
    this.enterKey = keyClass("Enter");
    this.escKey = keyClass("Escape");
  ...
  gameOverHandler() {
    ...
    if (this.hits > this.topScore) {
      //If score beats current best
  ...
  startGame() {
    ...
    this.gameActive = true; //Set game as started
  ...
  startGameTimer(delay, step) {
    console.log("Starting game start timer...")
    this.enterKey.reset()
    ...
    this.batton.reset(); //Reset Batton
  ...
  testCollision() {
    ...
    if (battonLeft > Math.PI) {
      //If left of batton is over the pi-line
      var angleTest =
        (-Math.PI < this.ball.positionAngle &&
          this.ball.positionAngle < battonLeft - 2 * Math.PI) ||
        (battonRight < this.ball.positionAngle &&
          this.ball.positionAngle < Math.PI);
    } else if (battonRight < -Math.PI) {
      //If right of batton is under the pi-line
      var angleTest =
        (-Math.PI < this.ball.positionAngle &&
          this.ball.positionAngle < battonLeft) ||
        (battonRight + 2 * Math.PI < this.ball.positionAngle &&
          this.ball.positionAngle < Math.PI);
    } else {
      //If away from the pi-line
      var angleTest =
        battonRight < this.ball.positionAngle &&
        this.ball.positionAngle < battonLeft;
    }
    ...
    // If the ball is near the edge
    if (radiusTest) {
      // If within the collision angle, or in godMode
      if (this.godMode || angleTest) {
  ...
  collisionHandler() {
    //If ball has colided with batton, or godMode is on
    ...
    if (
      absolute(Math.cos(this.ball.velocityAngle + this.ball.positionAngle)) >
      0.5
    ) {
      //For steep angles
      ...
      this.ball.velocityAngle =
        Math.PI -
        this.ball.velocityAngle -
        2 * this.ball.positionAngle -
        this.batton.direction *
          0.3 *
          cube(
            absolute(
              Math.cos(this.ball.velocityAngle + this.ball.positionAngle)
            )
          ) +
      ...
      if (absolute(this.ball.positionAngle) > 0.6 * Math.PI) {
        //For left half
        ...
        this.ball.velocityAngle =
          ...
          this.ball.velocityAngle -
          2 * this.ball.positionAngle -
          (this.ball.velocityAngle / absolute(this.ball.velocityAngle)) *
    ...
    this.ball.velocity = GetVectorV(
      this.ball.velocityRadius,
      this.ball.velocityAngle
    ); //Update velocity vector after collision, from magnitude and angle
    console.log("Final ball velocity: " + this.ball.velocity.vector());

    if (this.ball.positionRadius > this.R - this.ball.size) {
      //If position is greater than inner boundary
      this.ball.positionRadius =
        this.R - this.ball.size - this.ball.velocityRadius; //Set radius to within inner boundary (prevent getting stuck outside)
      this.ball.position = GetVector(
        this.ball.positionRadius,
        this.ball.positionAngle,
        this.x0,
        this.y0
      ); //Set new position in x and y
    }
  ...
  update() {
    if (!this.gameActive) {
      ...
      if (!this.gameOver) {
        //If not on gameovger screen, keep recalculating ball center position
        this.ball.position.x = this.x0; //Reset x
        this.ball.position.y = this.y0; //Reset y
      }
      ...
      if (!this.gameOver && !this.gamePaused) {
        ...
        // DEATH MODE
        if (20 < this.level && this.level <= 30) {
          // If in stage 2
